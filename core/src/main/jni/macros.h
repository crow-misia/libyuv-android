...
#define DECLARE_BUFFER_SRC_WITH_OFFSET(jbuf, buf, jOffset, offset, dummy_retval) \
  DECLARE_OFFSET(jOffset, offset, dummy_retval); \
  ScopedBytesRO scoped_##buf(env, jbuf); \
  const uint8_t *buf = reinterpret_cast<const uint8_t *>(scoped_##buf.get() + offset); \
  if (buf == nullptr) { \
    throwIllegalArgumentException(env, "ByteBuffer " #buf " is not available"); \
    return dummy_retval; \
  }
...
#define DECLARE_BUFFER_DST_WITH_OFFSET(jbuf, buf, jOffset, offset, dummy_retval) \
  DECLARE_OFFSET(jOffset, offset, dummy_retval); \
  ScopedBytesRW scoped_##buf(env, jbuf); \
  uint8_t *buf = reinterpret_cast<uint8_t *>(scoped_##buf.get() + offset); \
  if (buf == nullptr) { \
    throwIllegalArgumentException(env, "ByteBuffer " #buf " is not available"); \
    return dummy_retval; \
  }
...
#define DECLARE_OFFSET(jOffset, offset, dummy_retval) \
  int offset = jOffset; \
  if (offset < 0) { \
    throwIllegalArgumentException(env, "Offset " #offset " must be positive"); \
    return dummy_retval; \
  }
...
#define SRC_PLANE_OFFSET(name) \
  DECLARE_BUFFER_SRC_WITH_OFFSET(j_src_##name, src_##name, j_src_offset_##name, src_offset_##name,); \
  DECLARE_STRIDE(j_src_stride_##name, src_stride_##name,);
...
#define DST_PLANE_OFFSET(name) \
  DECLARE_BUFFER_DST_WITH_OFFSET(j_dst_##name, dst_##name, j_dst_offset_##name, dst_offset_##name,); \
  DECLARE_STRIDE(j_dst_stride_##name, dst_stride_##name,);
